...
function Clock() {
  ...
  return <span className="font-mono text-slate-300" style={{ fontSize: "clamp(1.25rem, 2.2vw, 2.5rem)" }}>{time}</span>;
}
...
function TvPage() {
  ...
  return (
    <div
      ...
      style={{ height: "100dvh", minHeight: "100vh", fontFamily: "Poppins, sans-serif" }}
    >
      {/* ── Header ── */}
      <header
      ...
      >
        ...
        <div className="flex items-center" style={{ gap: "clamp(1rem, 3vw, 3rem)" }}>
          <span className="text-slate-400 font-medium" style={{ fontSize: "clamp(1rem, 1.8vw, 1.75rem)" }}>
            {mesLabel}/{data.ano}
          </span>
          <Clock />
        </div>
      ...
      <div
        ...
        style={{ gap: "clamp(0.5rem, 1.2vw, 1.5rem)", padding: "clamp(0.5rem, 1vw, 1rem) clamp(1rem, 2vw, 2.5rem)" }}
      >
      ...
      <div
        ...
        style={{ padding: "0 clamp(0.75rem, 1.5vw, 2rem) clamp(0.25rem, 0.5vw, 0.75rem)" }}
      >
        {/* Left arrow */}
        <button
          onClick={goPrev}
          ...
          style={{ width: arrowSize, height: arrowSize, left: "clamp(0.25rem, 0.8vw, 1rem)" }}
        >
          <svg
            viewBox="0 0 24 24"
            fill="none"
            ...
            style={{ width: "clamp(1rem, 2vw, 2rem)", height: "clamp(1rem, 2vw, 2rem)" }}
          >
        ...
        <button
          ...
          style={{ width: arrowSize, height: arrowSize, right: "clamp(0.25rem, 0.8vw, 1rem)" }}
        >
          <svg
            viewBox="0 0 24 24"
            fill="none"
      ...
      <footer
        ...
        style={{
          padding: "clamp(0.3rem, 0.8vw, 0.75rem) clamp(1rem, 2.5vw, 3rem)",
          fontSize: "clamp(0.75rem, 1.2vw, 1.125rem)",
        }}
      >
        <span>Atualizado: {lastUpdate || "--:--"}</span>
        <div className="flex items-center" style={{ gap: "clamp(0.75rem, 2vw, 2rem)" }}>
          <div className="flex" style={{ gap: "clamp(0.4rem, 0.8vw, 0.75rem)" }}>
            {SLIDE_NAMES.map((name, i) => (
              <button
                ...
                style={{
                  padding: "clamp(0.2rem, 0.4vw, 0.35rem) clamp(0.6rem, 1.2vw, 1rem)",
          ...
          <span className="font-mono text-slate-600" style={{ minWidth: "3em", textAlign: "right" }}>
...
function KpiCard({ label, value, color }: { label: string; value: string; color: string }) {
  return (
    <div
      ...
      style={{ padding: "clamp(0.5rem, 1.2vw, 1.25rem) clamp(0.75rem, 1.5vw, 1.5rem)" }}
    >
      <p
        className="text-slate-400 uppercase tracking-wider font-medium"
        style={{ fontSize: "clamp(0.65rem, 1.1vw, 1rem)", marginBottom: "clamp(0.25rem, 0.5vw, 0.5rem)" }}
      >
        {label}
      </p>
      <p
        className={`font-bold leading-tight ${color}`}
        style={{ fontSize: "clamp(1.25rem, 3.2vw, 3.5rem)" }}
      >
        {value}
      </p>
    </div>
