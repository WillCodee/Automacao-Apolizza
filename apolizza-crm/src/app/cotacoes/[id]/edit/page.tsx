...
export default async function EditCotacaoPage({ params }: Params) {
  ...
  const initialData = {
    ...
    inicioVigencia: row.inicioVigencia ? (row.inicioVigencia instanceof Date ? row.inicioVigencia.toISOString().split("T")[0] : String(row.inicioVigencia)) : "",
    fimVigencia: row.fimVigencia ? (row.fimVigencia instanceof Date ? row.fimVigencia.toISOString().split("T")[0] : String(row.fimVigencia)) : "",
    primeiroPagamento: row.primeiroPagamento ? (row.primeiroPagamento instanceof Date ? row.primeiroPagamento.toISOString().split("T")[0] : String(row.primeiroPagamento)) : "",
    proximaTratativa: row.proximaTratativa ? (row.proximaTratativa instanceof Date ? row.proximaTratativa.toISOString().split("T")[0] : String(row.proximaTratativa)) : "",
