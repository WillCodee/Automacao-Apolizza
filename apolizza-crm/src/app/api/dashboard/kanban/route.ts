...
export async function GET(_req: NextRequest) {
  try {
    ...
    // Cotações agrupadas por usuário
    ...
    for (const row of groupRows.rows as Array<{ grupo_id: string; grupo_nome: string; grupo_cor: string | null; user_id: string }>) {
    ...
    for (const r of byUserRows.rows as CotRow[]) {
      const userId = r.assignee_id || "sem_responsavel";
      const userName = r.assignee_name || "Sem Responsável";
